...
export default function Hero() {
  return (
    <section id="top" className="relative min-h-screen overflow-hidden pt-28 md:pt-40 pb-20" data-testid="hero-section">
      {/* Background layers */}
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-8 lg:px-12">
        ...
        <h1 className="font-display font-black uppercase leading-[0.88] tracking-tighter text-white" data-testid="hero-headline">
          {HEADLINE.map((line, i) => (
            <div key={i} className="overflow-hidden">
              <motion.div
                initial={{ y: "110%" }} animate={{ y: 0 }}
                transition={{ duration: 0.9, delay: 0.1 + i * 0.09, ease: [0.22, 1, 0.36, 1] }}
                className="flex gap-[0.18em] text-[13vw] md:text-[10vw] lg:text-[8.2vw]"
              >
                {line.map((w, j) => (
        ...
        >
          ...
          <div className="md:col-span-7 flex flex-col sm:flex-row gap-4 sm:justify-end">
            <MagneticButton
              ...
              className="glow-btn bg-white text-black rounded-full px-8 py-4 font-semibold text-sm group"
            ...
            <MagneticButton
              ...
              className="rounded-full px-8 py-4 font-medium text-sm border border-white/20 text-white hover:bg-white hover:text-black transition-colors"
